...
const MODEL = 'glm-4.6:cloud'; // Vision model with cloud support
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (!response.response) {
            reject(new Error(`No response field in API response`));
            return;
          }
          ...
          reject(new Error(`Parse error for ${filename}: ${error.message}. Data: ${data.substring(0, 200)}`));
        }
      });
    });
...
async function processImage(filename, index, total) {
  ...
  try {
    ...
    // Read and resize image to max 800px width to reduce size for API
    ...
    console.log(`  Original file size: ${(fs.statSync(filePath).size / 1024).toFixed(2)} KB`);
    console.log(`  Resized for API: ${(imageBuffer.length / 1024).toFixed(2)} KB`);
    ...
    // Add delay to avoid rate limiting (important for cloud API)
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║  Dish Impex Photos Renaming Tool (Ollama Cloud Vision)  ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log(`\nModel: ${MODEL}`);
  console.log(`API Endpoint: https://ollama.com/api/generate`);
